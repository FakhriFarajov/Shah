...
export default function Navbar() {
    ...
    const [favouritesCount, setFavouritesCount] = useState<number>(() => {
        const stored = localStorage.getItem('favouritesCount');
        return stored ? Number(stored) : 0;
    });
    ...
    useEffect(() => {
        ...
        if (token) {
            try {
                ...
                const cartC = Number(decoded.cart_count) || 0;
                const favC = Number(decoded.favourite_count) || 0;
                setCartCount(cartC);
                setFavouritesCount(favC);
                localStorage.setItem('cartCount', String(cartC));
                localStorage.setItem('favouritesCount', String(favC));
            } catch (err) {
                tokenStorage.clear();
                setUserLogo("");
                setCartCount(0);
                ...
                localStorage.setItem('favouritesCount', '0');
            }
        }
        ...
        const fetchCounts = async () => {
            try {
                ...
                const cartC = cartArr.reduce((sum: number, it: any) => sum + (Number(it.quantity) || 1), 0);
                const favC = favArr.length;
                setCartCount(cartC);
                setFavouritesCount(favC);
                localStorage.setItem('cartCount', String(cartC));
                localStorage.setItem('favouritesCount', String(favC));
        ...
        const onFavouritesUpdated = (e: any) => {
            try {
                ...
                setFavouritesCount((c) => {
                    const newCount = Math.max(0, c + Number(delta));
                    localStorage.setItem('favouritesCount', String(newCount));
                    return newCount;
                });
            } catch (err) {
            }
