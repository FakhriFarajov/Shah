...
public class AddressService : IAddressService
{
    ...
    public async Task<Result> AddAddressAsync(AddAddressRequestDTO request)
    {
        ...
        var buyerProfile = await _context.BuyerProfiles.FirstOrDefaultAsync(a => a.Id == request.BuyerId);
        if (buyerProfile == null)
            return Result.Error("BuyerProfile not found", 404);

        var addressExists = await _context.Addresses.AnyAsync(a => a.BuyerProfileId == request.BuyerId);
        if (addressExists)
            return Result.Error("Address already exists for this buyer", 409);

        var newAddress = _mapper.Map<Address>(request);
        newAddress.BuyerProfileId = request.BuyerId;
        buyerProfile.AddressId = newAddress.Id;
        _context.Update(buyerProfile);
    ...
    public async Task<Result> EditAddressAsync(EditAddressRequestDTO request)
    {
        ...
        var buyerProfile = await _context.BuyerProfiles.FirstOrDefaultAsync(b => b.Id == request.BuyerId);
        if (buyerProfile == null)
            return Result.Error("BuyerProfile not found", 404);

        var address = await _context.Addresses.FindAsync(request.AddressId);
        if (address == null || address.BuyerProfileId != request.BuyerId)
            return Result.Error("Address not found for this buyer", 404);
