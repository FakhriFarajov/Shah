...
export default function OrdersPage() {
	...
	useEffect(() => {
		const fetchWarehouseOrders = async () => {
			setLoading(true);
			try {
				const response: PaginatedResult<Warehouse> = await apiCallWithManualRefresh(() => getAllPaginatedWarehouses(page, pageSize));
											...
											<div className="flex flex-wrap gap-4">
												{order.items && order.items.length > 0 ? (
													order.items.map((item: any) => (
														<div key={item.id} className="bg-gray-50 rounded-xl border p-4 flex flex-col items-center w-104">
