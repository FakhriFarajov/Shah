...
namespace ShahAdminFeaturesApi.Application.Validators
{
    public class SellerEditRequestValidator : AbstractValidator<EditSellerRequestDTO>
    {
        public SellerEditRequestValidator()
        {
            ...
            // PassportNumber: optional, alphanumeric, reasonable length
            RuleFor(x => x.PassportNumber)
                .Matches("^[A-Za-z0-9]+$").WithMessage("Passport number must contain only letters and digits.")
                .MinimumLength(3).WithMessage("Passport number must be at least 3 characters.")
