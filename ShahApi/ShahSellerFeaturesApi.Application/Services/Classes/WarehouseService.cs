...
public class WarehouseService : IWarehouseService
{
    ...
    // Corrected: returns order items after assigning order to warehouse
    public async Task<TypedResult<object>> AssignOrderItemsToWarehouseAsync(string warehouseId, string orderId)
    {
        ...
        if (existingLink != null)
        {
            ...
            var alreadyItems = order.OrderItems.Select(oi => new
            {
                ...
                VariantPrice = (oi.ProductVariant != null && oi.ProductVariant.Price > 0) ?
                    ...
                    : 0,
                Amount = ((oi.ProductVariant != null && oi.ProductVariant.Price > 0) ?
                    ((oi.ProductVariant.DiscountPrice > 0 && oi.ProductVariant.DiscountPrice < oi.ProductVariant.Price) ? oi.ProductVariant.DiscountPrice : oi.ProductVariant.Price)
                    : 0) * oi.Quantity,
        ...
        // Check current number of assigned orders
        var assignedOrderCount = await _context.WarehouseOrders.CountAsync(wo => wo.WarehouseId == warehouseId);
        if (assignedOrderCount >= warehouse.Capacity)
            return TypedResult<object>.Error("Warehouse has reached its capacity. No place available.", 400);
