...
import type {BuyerProfileResponseDTO, EditBuyerRequestDTO, PaginatedBuyersResponse } from "@/features/profile/DTOs/buyer.interfaces";


// Buyer profile
export async function getAllBuyerProfilesPaginated(pageNumber: number, pageSize: number = 5): Promise<PaginatedBuyersResponse> {
  const response  = await authHttp.get<PaginatedBuyersResponse>(`/getAll`, {
